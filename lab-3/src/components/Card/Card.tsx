...
function Card(card: ICard) {
  const [liked, setLiked] = useState(favoriteStore.isLiked(card));
  const onClick = () => {
    setLiked((prev) => !prev);
    if (liked) {
      favoriteStore.removeFavorite(card);
    } else {
      favoriteStore.addFavorite(card);
    }
  };
  return (
    <div className={classes.card}>
      ...
      <Link
        to={`/${card.cathegory}/${String(card.id)}`}
